...
\lstdefinelanguage{SEMINARXSD}
{morekeywords={xs,schema,xmlns,element,name,complexType,sequence,simpleType,restriction,base,pattern,value,attribute},
sensitive=false
}

\lstdefinelanguage{SEMINARXML}
{morekeywords={seminar,sem_date,sem_time,sem_url,sem_title,sem_room,sem_url,sem_duration,by},
sensitive=false
}

\lstdefinelanguage{SEMINARXSLT}
...
\begin{figure}
\lstset{language=SEMINARXSD,basicstyle=\ttfamily}
{\begin{lstlisting}
...
\begin{figure}
\lstset{language=SEMINARXML,basicstyle=\ttfamily}
{\begin{lstlisting}
<?xml version="1.0"?>
<seminar by="m_bkarak">
...
\begin{figure}
\lstset{language=SEMINARXSLT,basicstyle=\ttfamily}
{\begin{lstlisting}
...
$Id$
-->
<title>ELTRUN Seminars</title>
</head>
<body>
<a href="http://www.eltrun.gr/">
<img src="../images/heading.jpg" 
 alt="ELTRUN - The e-Business Center" border="0">
</a><br><table border="0">
<tbody valign="top">
<tr>
...
<a name="20040314"></a>
	    Presenter: <a href="../members/m_bkarak.html">Mr. Vassilios Karakoidas</a><br>
	    Date:
	    14 March 2004<br>
	    Time: 19:00<br>
	    Duration: 3 hrs<br>
	    Location: 906<br><br><a href="http://www.eltrun.gr/seminar/presentations.ppt">
		Download the presentation
	    </a>
</td>
</tr>
...
\begin{description}
\item[Maintain schemas, transformations and users] This use case describes the maintainence and development procedures of the system. Each Content Developer can update the DTD schemas, XSL scripts and the make file in order to correct errors or add new features.

\item[Update local repository] Each user in the system must update the local repository each time he uses the system.

\item[Commit changes] Content developers and administrators must commit the changes from their local repository to the CVS repository in the web site server.

\item[Maintain content] Content Developers maintain the content of the web site. Content consists of XML data and bibtex collection files.

\item[Generate web site] Content administrators and developers can generate the web site localy for preview. 
