...
XML is a simple text format derived from SGML (Standard Generalized Markup Language). XML 
provides a hierachical model to describe data \cite{W3C_XML}. For data transformations we 
implemented a system based on XSLT \cite{W3C_XSLT}. XSLT is a language for tranforming XML documents.
transforming XML.\\
BibTex is a program and file format that allows the management of bibliographic entries. Initialy 
designed by Oren Patashnik and Leslie Lamport for the LaTeX document preparation system. \cite{Pa88} \cite{Ja99}.\\
To deal with the problem of versioning and multi-persion development we used CVS (Concurent Version System) \cite{CVS}. 
CVS main usage is for keeping track of text files.\\
The simple tasks of our system were automated using a make file. Make \cite{gnu_make} a tool 
